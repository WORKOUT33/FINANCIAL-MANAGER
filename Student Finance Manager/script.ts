...
const trasactionHistory = document.querySelector(
  ".transaction > div"
) as HTMLDivElement;
...
const addMoneyContainer = document.querySelector(
  ".add-money-container"
) as HTMLDivElement;
const btnCancelForm = document.querySelector(
  ".btn-Cancel-form"
) as HTMLButtonElement;
const addAmountInput = document.querySelector(
  "#Add-amount"
) as HTMLInputElement;
const addMoneyForm = document.querySelector(
  ".add-money-form"
) as HTMLFormElement;
...
const btnReminder = document.querySelector(
  ".btn-reminder"
) as HTMLButtonElement;
const reminderContainer = document.querySelector(
  ".reminder-container"
) as HTMLDivElement;
const btnCancelReminder = document.querySelector(
  ".btn-Cancel-reminder"
) as HTMLButtonElement;
...
const billsBalance = document.querySelector(
  ".BU-balance span"
) as HTMLSpanElement;
const foodAndDrinkBalance = document.querySelector(
  ".FD-balance span"
) as HTMLSpanElement;
const otherBalance = document.querySelector(".other span") as HTMLSpanElement;
const schoolBalance = document.querySelector(
  ".School-balance span"
) as HTMLSpanElement;
const checkboxInput = document.querySelector("#checkbox") as HTMLInputElement;
const setReminderForm = document.querySelector(
  ".set-reminder-form"
) as HTMLFormElement;
...
function loadReminders(): Reminder[] {
  const retrievedReminders = localStorage.getItem(
    `reminder_${currentUserEmail}`
  );
...
function loadTransactions(): Transactions[] {
  ...
  return loadedValue
    ? JSON.parse(loadedValue).map((trans: Transactions) => ({
        ...trans,
...
const totalSchoolFeesBalance = schoolFees.reduce(function (
  acc: number,
  curr: number
) {
  return acc + curr;
...
const totalFoodAndDrinkBalance = foodAndDrink.reduce(function (
  acc: number,
  curr: number
) {
  return acc + curr;
...
const totalBills = billsAndUtilities.reduce(function (
  acc: number,
  curr: number
) {
  return acc + curr;
  ...
  const html = `
                     <div>
                            <div>
                                ...
                                <input type="checkbox" ${
                                  reminder.complete ? "checked" : ""
                                } onchange="toggleReminderComplete('${
...
function renderTransaction(trans: Transactions): void {
  const html = `
                     <div class="transaction-item">
                        <span class="cat ${trans.category}">${
    trans.category
  }</span>
                        <span class="amt">ZMW ${trans.amount.toFixed(2)}</span>
                        <span class="date">${trans.date.getDate() + 1}:${
    trans.date.getMonth() + 1
  }:${trans.date.getFullYear()}</span>
                    </div>
                    <hr> 
                `;
...
function updateSchoolFeesBalance(): void {
  const totalSchoolFeesBalance = schoolFees.reduce(
    (acc, curr) => acc + curr,
    0
  );
...
function updateFoodAndDrinkBalance(): void {
  const totalFoodAndDrinkBalance = foodAndDrink.reduce(
    (acc, curr) => acc + curr,
    0
  );
...
function updateReminderStorage() {
  localStorage.setItem(
    `reminder_${currentUserEmail}`,
    JSON.stringify(reminderArray)
  );
}

function updateTransactionStorage() {
  localStorage.setItem(
    `transaction_${currentUserEmail}`,
...
function updateSchoolStorage() {
  localStorage.setItem(
    `school_${currentUserEmail}`,
    JSON.stringify(schoolFees)
  );
}

function updateBillAndUtilitiesStorage() {
  localStorage.setItem(
    `bills_${currentUserEmail}`,
    JSON.stringify(billsAndUtilities)
  );
}

function updateFoodAndDrinkStorage() {
  localStorage.setItem(
    `food_${currentUserEmail}`,
    JSON.stringify(foodAndDrink)
  );
}
...
function deleteReminder(description: string): void {
  const index = reminderArray.findIndex(
    (rem) => rem.description === description
  );
  if (index > -1) {
...
const totalIncome =
  balance.reduce(function (acc, curr) {
    return acc + curr;
  }, 0) -
  totalBills -
